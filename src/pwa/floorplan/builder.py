...
def parse_run(
    *,
    ...
    try:
        ...
        validate_contained_destination(runs_root, parse_run_id)
        validate_contained_destination(runs_root, Path(".staging") / parse_run_id)
    except (ValueError, OSError):
        diagnostic = _diagnostic(
            parse_run_id=parse_run_id,
            source_run_id=None,
    ...
    try:
        destination_exists = final_run.exists() or staging_run.exists()
    except OSError:
    ...
    try:
        ...
        source_quality = json.loads(quality_bytes.decode("utf-8"))
    except (ValueError, OSError, UnicodeDecodeError, json.JSONDecodeError, RecursionError):
        diagnostic = _diagnostic(
            parse_run_id=parse_run_id,
            source_run_id=None,
            adapter=adapter,
    ...
    manifest_or_quality_invalid = not isinstance(source_manifest, dict) or not isinstance(source_quality, dict)
    if not manifest_or_quality_invalid:
        try:
    ...
    if manifest_or_quality_invalid:
        diagnostic = _diagnostic(
            parse_run_id=parse_run_id,
            source_run_id=source_manifest.get("run_id") if isinstance(source_manifest, dict) else None,
            adapter=adapter,
            cli_exit=2,
            outcome="operational",
            finding=None,
    ...
    if source_quality["content_hash"] != compute_content_hash(source_quality):
        ...
        return ParseRunResult(cli_exit=2, final_run=final_run, staging_run=staging_run, diagnostic=diagnostic)
    inventory = source_manifest["payload"]["inputs"]
    floorplan_entries = [item for item in inventory if item["kind"] == "floorplan"]
    inventory_paths = [item["path"] for item in inventory]
    source_identity_invalid = (
        source_manifest["project_id"] != source_quality["project_id"]
        or source_manifest["run_id"] != source_quality["run_id"]
        or source_manifest["run_id"] != source_run.name
        or len(floorplan_entries) != 1
        or len(inventory_paths) != len(set(inventory_paths))
    )
    if source_identity_invalid:
        diagnostic = _diagnostic(
            parse_run_id=parse_run_id,
            source_run_id=source_manifest["run_id"],
            adapter=adapter,
    ...
    for item in source_manifest["payload"]["inputs"]:
        try:
            resolve_contained_relpath(source_run, item["path"])
        except (ValueError, OSError):
            diagnostic = _diagnostic(
                parse_run_id=parse_run_id,
                source_run_id=source_manifest["run_id"],
            ...
            return ParseRunResult(cli_exit=2, final_run=final_run, staging_run=staging_run, diagnostic=diagnostic)
    floorplan_entry = floorplan_entries[0]
    try:
    ...
    if annotation is not None:
        ...
        if len(annotation_bytes) > MAX_ANNOTATION_BYTES:
            ...
            return ParseRunResult(cli_exit=2, final_run=final_run, staging_run=staging_run, diagnostic=diagnostic)
        try:
            annotation_document = json.loads(annotation_bytes.decode("utf-8"))
        ...
        annotation_invalid = not isinstance(annotation_document, dict)
        if not annotation_invalid:
            try:
                annotation_invalid = bool(validate_artifact(annotation_document))
            except (ValueError, KeyError):
        ...
        if not DxfSource().accepts(source_floorplan):
            ...
            return ParseRunResult(cli_exit=2, final_run=final_run, staging_run=staging_run, diagnostic=diagnostic)
        try:
            source_floorplan_size = source_floorplan.stat().st_size
        except OSError:
            diagnostic = _diagnostic(
                parse_run_id=parse_run_id,
                source_run_id=source_manifest["run_id"],
            ...
            return ParseRunResult(cli_exit=2, final_run=final_run, staging_run=staging_run, diagnostic=diagnostic)
        if source_floorplan_size > MAX_DXF_BYTES:
    ...
    try:
        staging_run = create_contained_directory(runs_root, Path(".staging") / parse_run_id)
        create_contained_directory(staging_run, "project")
        create_contained_directory(staging_run, "parse")
    except (OSError, ValueError):
        diagnostic = _diagnostic(
            parse_run_id=parse_run_id,
            source_run_id=source_manifest["run_id"],
    ...
    try:
        ...
        if annotation is not None:
            ...
            source_image = resolve_contained_relpath(
                staging_run,
                annotation_document["payload"]["image"]["source_image_ref"],
            )
            ...
            raw = DxfSource().extract(staged_floorplan)
            source_image = staged_floorplan
        ...
        # exclusive write fails, the operational filesystem handler further down
