...
- Status: **`ACCEPTED (G1 claimed)`** — implemented, reviewed across five independent rounds, and the final §20 visual/geometry gate approved by Moshe on 2026-08-13 (gate `t_f442d3dc`). G1 evidence is sufficient and the parser output may be handed to PLAN-003. Revision 2 (projected opening width, sanitised raster overlay) and the GC3-8 PDF-page contract amendment and the AC-13 enumeration are all approved and implemented.
