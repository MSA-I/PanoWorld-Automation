...
def test_fixture_without_independent_truth_or_scale_anchors_fails_closed(tmp_path: Path) -> None:
    ...
    assert result["determinism"]["replays"] == 2
    assert result["determinism"]["identical"] is True
    assert result["runtime"]["max_seconds"] < 60.0
    assert result["memory"]["peak_tracemalloc_bytes"] < 1_610_612_736
